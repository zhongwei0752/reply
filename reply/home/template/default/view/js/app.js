...
var ArticleAnimator = ArticleAnimator || {
  ...
  postCount:          getUrlParam('count'),
  currentPostIndex:   getUrlParam('id'),
  postCache:          {},
  pageTemplate:       null,
};
...
ArticleAnimator.load = function(){
  ...
  this.createPost({ type: 'current' }, function(){
    self.createPost({ type: 'next' }, function(){
      ...
      history.pushState(pageState(), "", "space.php?do=reply&count="+self.postCount+"&id=" + self.currentPostIndex)
...
ArticleAnimator.getPost = function(index, callback){
  ...
  var self = this;
  $.getJSON('http://localhost/reply/home/capi/space.php?do=reply&id='+ index +'', function(d){
...
function getURLIndex(){
  return parseInt( (history.state && history.state.index) ||window.location.hash.replace('space.php?do=reply&id=', "") || ArticleAnimator.currentPostIndex );
}
