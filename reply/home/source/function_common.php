...
function ckprivacy($type, $feedmode=0) {
	...
	$_SGLOBAL[$var] = $result;//µ±Ç°Ò³Ãæ»º´æ
	return $result;
}
	...
	if(!isset($_SGLOBAL[$var])) {
		if(empty($_SGLOBAL['supe_uid'])) {
			...
			if(substr($permtype, 0, 6) == 'manage' && empty($_SGLOBAL[$var])) {
				$_SGLOBAL[$var] = $_SGLOBAL['usergroup'][$gid]['manageconfig'];//È¨ÏÞ¸²¸Ç
				if(empty($_SGLOBAL[$var])) {
					$_SGLOBAL[$var] = ckfounder($_SGLOBAL['supe_uid'])?1:0;//´´Ê¼ÈË
				}
			}
		}
	}
...
function multi($num, $perpage, $curpage, $mpurl, $ajaxdiv='', $todiv='') {
	...
	if($num > $perpage) {
		...
		if($curpage - $offset > 1 && $pages > $page) {
			...
			$multipage .= " class=\"active\" title=\"页头\">&lsaquo;&lsaquo;&lsaquo;</a>";
		}
		if($curpage > 1) {
			$multipage .= "<a ";
			if($_SGLOBAL['inajax']) {
			...
			$multipage .= " class=\"prev\" title=\"上一页\">&lsaquo;&lsaquo;</a>";
		}
		for($i = $from; $i <= $to; $i++) {
			if($i == $curpage) {
		...
		if($curpage < $pages) {
			...
			$multipage .= " class=\"next\" title=\"下一页\">&rsaquo;&rsaquo;</a>";
		}
		if($to < $pages) {
			$multipage .= "<a ";
			if($_SGLOBAL['inajax']) {
			...
			$multipage .= " class=\"last\" title=\"页尾\">&rsaquo;&rsaquo;&rsaquo;</a>";
		}
		//$multipage .= "<a title=\"总页数\">".$num."页</a>";
	}
	return $multipage;
}
...
function ckfriend($touid, $friend, $target_ids='') {
	...
	//×Ô¼º
	if($touid == $_SGLOBAL['supe_uid']) return true;//×Ô¼º
...
function realname_set($uid, $username, $name='', $namestatus=0) {
	...
	if($name) {
		...
		$_SN[$uid] = $username;
		$_SGLOBAL['select_realname'][$uid] = $uid;//ÐèÒª¼ìË÷
	}
}
...
function getmtag($id) {
	...
	//³ÉÔ±¼¶±ð
	$mtag['ismember'] = 0;
	$mtag['grade'] = -9;//-9 ·Ç³ÉÔ± -2 ÉêÇë -1 ½ûÑÔ 0 ÆÕÍ¨ 1 Ã÷ÐÇ 8 ¸±ÈºÖ÷ 9 ÈºÖ÷
...
function ckspacelog() {
	...
	if($value = $_SGLOBAL['db']->fetch_array($query)) {
		if($value['expiration'] && $value['expiration'] <= $_SGLOBAL['timestamp']) {//µ½ÆÚ
